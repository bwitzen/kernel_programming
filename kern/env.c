...
void env_init(void)
{
    // reverse loop to ensure freelist is in required order
    for (uint32_t i = 0; i < NENV; i++) {
        uint32_t inv = (NENV - 1) - i;
        envs[inv].env_status = ENV_FREE;
        envs[inv].env_id = 0;
        envs[inv].env_link = env_free_list;
        env_free_list = &envs[inv];
    }
...
static int env_setup_vm(struct env *e)
{
    ...
    // simply copying the kernel pagedir to environment works fine
    p->pp_ref += 1;
...
int env_alloc(struct env **newenv_store, envid_t parent_id)
{
    ...
    e->env_time_slice = ENV_TIME_SLICE;
    e->env_wait_env = ENV_NOT_WAITING;
    e->env_runs = 0;
...
{
    // rounding as decribed above
    va = ROUNDDOWN(va, PGSIZE);
    len = (size_t) ROUNDUP(len, PGSIZE);

    // physical page allocation and virtual linking
    for (size_t i = 0; i < len; i += PGSIZE) {
        struct page_info *pp = page_alloc(0);
        if (!pp)
            panic("region_alloc: could not allocate.\n");
        page_insert(e->env_pgdir, pp, va + i, PTE_W | PTE_U);
    }
}
...
static void load_icode(struct env *e, uint8_t *binary)
{
    ...
    // the code here is copied from boot/main.c
    struct elf_proghdr *ph, *eph;
    struct elf *p = (struct elf *) binary;

    if (p->e_magic != ELF_MAGIC)
     		panic("load_icode: this is not an ELF executable");

    ph = (struct elf_proghdr *) ((uint8_t *) p + p->e_phoff);
    eph = ph + p->e_phnum;

    // goto env addressing
    lcr3(PADDR(e->env_pgdir));

    // copy over the elfblocks
    for (; ph < eph; ph++) {
        ...
        {
          // [TODO]
          // the enclosed code is to circumvent a bug with our pagefault
          // handler (kern/trap.c) which appears to have a bug
          // the result is that the VMA still maintains an entry for all mem
          // including BINARY, but will never have to resolve a BINARY page-
          // fault. The VMA works fine for all ANON memory
        ...
        vma_new(e, (void *) ph->p_va, ph->p_memsz, ph->p_flags, ph, binary);
    }

    // program's initial stack
    vma_new(e, (void *) (USTACKTOP - PGSIZE), PGSIZE, PTE_W, NULL, NULL);

    // set entry point
    e->env_tf.tf_eip = p->e_entry;
}
...
void env_create(uint8_t *binary, enum env_type type)
{
    ...
    // initialize VMA for this environment
    vma_init(env);
    env->env_time_slice = ENV_TIME_SLICE;

    // load in binary
  	load_icode(env, binary);

    // prepare two hardcoded VMA blocks, required for Lab4
    vma_new(env, UTEMP,          PGSIZE, 0,     NULL, NULL);
    vma_new(env, UTEMP + PGSIZE, PGSIZE, PTE_W, NULL, NULL);
...
void env_run(struct env *e)
{
    ...
    // do nothing if this is not a context switch
    if (curenv != e) {
        // step 1
